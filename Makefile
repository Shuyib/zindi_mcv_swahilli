...
venv/bin/activate: requirements.txt
	# create virtual environment used py3.10
	python3 -m venv .venv
...
run: 
	# run application
  	@echo "Download libsndfile1"
	sudo apt-get install libsndfile1 &&\
	@echo "Install Seamless m4t-medium and large you need over 32GB of RAM to run do all this in runpod the MCV dataset is huge"
	@echo "Convert mp3 to wav, transcribe wav to text, clean text, and save to csv"
	bash prepare_files.sh &&\
		python asr.py &&\
		python clean_submissions.py
	@echo "Process complete! Please check the output folder for the results."
